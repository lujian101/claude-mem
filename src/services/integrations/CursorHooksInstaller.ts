...
export function configureCursorMcp(target: CursorInstallTarget): number {
  ...
  if (!mcpServerPath) {
    ...
    console.error('   Expected at: ~/.claude/plugins/marketplaces/lujian101/plugin/scripts/mcp-server.cjs');
    return 1;
  }
...
export async function installCursorHooks(target: CursorInstallTarget): Promise<number> {
  ...
  if (!workerServicePath) {
    ...
    console.error('   Expected at: ~/.claude/plugins/marketplaces/lujian101/plugin/scripts/worker-service.cjs');
    return 1;
  }
